...
from flask import Flask, render_template, jsonify, request, Response, redirect, url_for, session, flash, abort
from flask.sessions import SessionInterface, SessionMixin
from pymongo import MongoClient, errors
from bson import ObjectId
from datetime import datetime, timedelta
# Added prometheus_flask_exporter library, which allows Flask apps to expose Prometheus metrics.
...
QUIZ_REQUEST_COUNT = Counter(
    "quiz_requests_total",
    "Number of requests to the quiz app",
    ["endpoint"]
)

HEALTH_STATUS = Gauge(
    "flask_health_status",
    "Status of the /healthz endpoint (1 = healthy, 0 = unhealthy)"
)

READY_STATUS = Gauge(
    "flask_ready_status",
    "Status of the /ready endpoint (1 = ready, 0 = not ready)"
)

HEALTH_CHECK_COUNT = Counter(
    "flask_health_check_total",
    "Total number of /healthz checks"
)

READY_CHECK_COUNT = Counter(
    "flask_ready_check_total",
    "Total number of /ready checks"
)
...
# Fetch Prometheus authentication credentials from environment variables
PROMETHEUS_USERNAME = os.getenv("PROM_USER", "prometheus")
PROMETHEUS_PASSWORD = os.getenv("PROM_PASS", "securepassword")
...
# Continuation of Definition Prometheus metrics
@app.before_request
def start_timer():
...
def log_request(response):
    """ Log request metrics with HTTP status code """
    request_latency = time.time() - request.start_time
    method = request.method
    endpoint = request.path
    http_status = str(response.status_code)  # Convert to string for Prometheus labels

    # Increment counters
    REQUEST_COUNT.labels(method=method, endpoint=endpoint, http_status=http_status).inc()
    REQUEST_LATENCY.labels(method=method, endpoint=endpoint, http_status=http_status).observe(request_latency)

    # Count quiz-related requests
    if request.endpoint and "quiz" in request.endpoint:
        QUIZ_REQUEST_COUNT.labels(endpoint=request.endpoint).inc()

    return response

@app.route("/metrics")
def prometheus_metrics():
    """Restrict access to Prometheus metrics using Basic Auth"""
    auth = request.authorization
    if not auth or auth.username != PROMETHEUS_USERNAME or auth.password != PROMETHEUS_PASSWORD:
        return Response("Unauthorized", status=401, headers={"WWW-Authenticate": "Basic realm='Login Required'"})
    
    return Response(generate_latest(), mimetype=CONTENT_TYPE_LATEST)

# Health and Readiness Endpoints with Metrics
@app.route('/healthz')
def health():
    HEALTH_STATUS.set(1)
...
@app.route('/ready')
def ready():
    READY_CHECK_COUNT.inc()
    try:
        client.server_info()  # Force connection on a request.
        READY_STATUS.set(1)  # Ready if MongoDB is reachable
        return jsonify({"status": "READY"}), 200  #'READY', 200
    except errors.ServerSelectionTimeoutError:
        READY_STATUS.set(0)  # Not ready if MongoDB is unreachable
