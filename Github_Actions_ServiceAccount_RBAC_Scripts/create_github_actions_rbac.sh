...
kubectl apply -f github_actions_rbac_config.yaml
...
if [ -z "$SECRET_NAME" ]; then
  ...
  if [ -z "$SECRET_TOKEN" ]; then
    ...
    echo "Secret token for the ServiceAccount github-actions-svc:"
    echo "$SECRET_TOKEN"
  fi
fi

echo "Github-actions service account and RBAC resources created successfully!"
